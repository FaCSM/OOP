...
TParser::TParser(QWidget *parent) : QWidget(parent) {
    ...
    while (response != 1 && response != -1) {
        ...
        graph = new Graph();
        response = prsFile(path);
        if (response != 1) showErr(response);
    }
}
...
std::string TParser::getPath() {
    QFile file = QFileDialog::getOpenFileName(
            nullptr, tr("Open input file"), "../../practice08", tr("Text (*.txt)")
    );
...
short TParser::prsFile(const std::string &) {
...
void TParser::showErr(const short &response) {
